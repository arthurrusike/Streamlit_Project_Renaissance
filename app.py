...
from helper_functions import style_dataframe, run_sql_query, sub_category_classification,load_profitbility_Summary_model
...
default_startDate = datetime.date(today.year-1, today.month, today.day)
default_endDate = datetime.date(today.year, today.month, today.day)
...
DetailsTab, AboutTab = st.tabs(["📊 Project R - Review",
                                         # "🥇 Site Detailed View",
...
try:
    ...
    with (DetailsTab):
            space_holder3,title_holder, cost_centres_selection  = st.columns(3)

            with space_holder3.expander("Customer Profitability Summary File", expanded=False):

                uploaded_file = st.file_uploader('Upload Customer Profitability Summary File',
                                            type=['xlsx', 'xlsm', 'xlsb'], key= 195)
            ...
            with fin2:
                ...
                kpi_key_metrics_data_pivot_Hume_KPI = key_metrics_data_Hume_KPI[
                    ["Capacity","Pallets", "Occ-%", "Rev | Plt","Rent | Plt", "EBITDA | Plt", "Turn", "DL %", "LTR %", "Rev | SQM",]]
            ...
            with st.expander("Customer Invoicing Comparison - Raw SWMS Invoicing Data for 2024", expanded=False):
                try:
                    ...
                    date_holder, start_date_range, end_date_range,country_label, refreshButton ,date_holder2  = st.columns((4,1,1,1,1,4))

                    startDate = start_date_range.date_input("Start Date", default_startDate, format="YYYY-MM-DD")
                    endDate = end_date_range.date_input("End Date", default_endDate, format="YYYY-MM-DD")
                    ...
                    if  refreshButton.button("Refresh", type="secondary",icon="🔃"):
                        invoice_rates = run_sql_query(startDate,endDate)
                    else:
                        invoice_rates = run_sql_query(startDate,endDate)

                    invoice_rates["Country"] = ["Australia" if " AU - " in x else "New Zealand" for x in invoice_rates.Cost_Center ]
                    ...
                    with(invoice_rates_by_service):
                        ...
                        weeks_ago_date = date_three_weeks_ago(today)
                        default_3_weeks_ago_startDate = datetime.date(weeks_ago_date.year, weeks_ago_date.month, weeks_ago_date.day)
                        invoice_rates_by_service_data = invoice_rates[invoice_rates.InvoiceDate > str(default_3_weeks_ago_startDate) ]

                        select_Option1_service, select_Option2_service = st.columns((1.5,1))

                        with select_Option1_service:
                            sel1_service, sel2_service, sel3_service, sel4b_service = st.columns((1.25, 1, 1,0.5))

                            selected_cost_centre_sel1 = sel1_service.selectbox("Cost Centre :", cost_centres, index=0, key=1115)

                            selected_workday_customers = invoice_rates_by_service_data[
                                invoice_rates_by_service_data.Cost_Center == selected_cost_centre_sel1].sort_values(
                                by="WorkdayCustomer_Name").WorkdayCustomer_Name.unique()

                            select_CC_data = invoice_rates_by_service_data[invoice_rates_by_service_data.Cost_Center == selected_cost_centre_sel1]
                            ...
                            Workday_Sales_Item_Name = invoice_rates_by_service_data.Workday_Sales_Item_Name.unique()
                            ...
                            selected_customer_pivot.WorkdayCustomer_Name =     selected_customer_pivot.WorkdayCustomer_Name.apply(proper_case)
                        ...
                        with select_Option2_service:
                            ...
                            st.subheader("2024 Invoiced Customer Service Rates Comparison", divider='rainbow')
                    ...
                    err_2.markdown(f"""
                                    ...
                                    Please Log on to via OKTA and activate Global Protect. \n
            ...
            st.subheader("Customer Account Rates Invoiced at Site ", divider="rainbow")

            _,weeks_input,_ = st.columns((3,1,3))
            weeks_ago = weeks_input.number_input(f"Invoiced at Site - Last Number of Weeks", value=4)

            changed_weeks_ago_date = date_three_weeks_ago(today,number_of_weeks=weeks_ago)

            default_3_weeks_ago_startDate = datetime.date(changed_weeks_ago_date.year, changed_weeks_ago_date.month, changed_weeks_ago_date.day)
            ...
            selected_site_rate_cards = v1.selectbox("Site Selection for Rate Cards :", site_list_rates, index=3)
    ...
    #             customer_avg_plts = selected_customer[selected_customer['Revenue_Category'] == 'Storage - Renewal'][
    #                 "Quantity"].mean()
    #             site_avg_plts_occupied = invoice_rates[invoice_rates['Revenue_Category'] == 'Storage - Renewal'][
    #                 "Quantity"].mean()
    #
    #             selected_customer_pivot_table = selected_customer_pivot.style.format({
    #                 "LineAmount": "${0:,.2f}",
